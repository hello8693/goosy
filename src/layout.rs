...
impl Layout {
    pub fn new(lines: &[LyricLine], height: f32, step_y: f64) -> Self {
        ...
        let pos_y = lines.iter().enumerate().map(|(index, _)| Spring::with_parameters(center + (index as f64 - active_idx as f64) * step_y, POS_STIFFNESS, POS_DAMPING, 1.0, true)).collect();
        let scale = lines.iter().map(|_| Spring::with_parameters(1.0, 100.0, 10.0, 1.0, true)).collect();
        Self { pos_y, scale, active_idx, height: height as f64, initialized: false, step_y }
    }

    pub fn update(&mut self, lines: &[LyricLine], t_ms: u64, fps: u32) {
        if lines.is_empty() || fps == 0 { return; }
        ...
        if !self.initialized {
            ...
            for index in 0..lines.len() {
                ...
                self.scale[index].set_target_position(if index == self.active_idx { 1.0 } else { 0.97 });
            }
        }
        let dt = 1.0 / fps as f64;
        for spring in &mut self.pos_y { spring.update(dt); }
        for spring in &mut self.scale { spring.update(dt); }
    }

    pub fn active_idx(&self) -> usize { self.active_idx }
...
fn active_index(lines: &[LyricLine], t_ms: u64) -> usize {
    lines.iter().position(|line| line.start_ms <= t_ms && t_ms < line.end_ms).unwrap_or_else(|| {
        lines.iter().rposition(|line| line.start_ms <= t_ms).unwrap_or(0)
    })
}
